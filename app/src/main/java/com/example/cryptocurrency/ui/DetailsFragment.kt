...
import com.example.cryptocurrency.utils.Constants.animationDuration
                    ...
                    when (it.status) {
                        ...
                        DataStatus.Status.SUCCESS -> {
                            it.data?.let { data ->
                                ...
                                tvCurrentPrice.text = data.marketData?.currentPrice?.usd?.roundToThreeDecimals()

                                val number = data.marketData?.priceChangePercentage24h?.roundToTwoDecimals()
                                ...
                                //GenesisDate
                                tvGenesisDate.text = if (!data.genesisDate.isNullOrEmpty()) data.genesisDate else "-"
